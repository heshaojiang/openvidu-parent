...
public class RpcHandler extends DefaultJsonRpcHandler<JsonObject> {
	...
	@Autowired
	OpenviduConfig openviduConfig;//配置类对象

	@Autowired
	GeoLocationByIp geoLocationByIp;//ip地址转换对象

	@Autowired
	SessionManager sessionManager;//session管理类

	@Autowired
	RpcNotificationService notificationService;//通知服务类
	/**
	 * 存放sessionId，客户端是否中断
	 */
	...
	public void handleRequest(Transaction transaction, Request<JsonObject> request) throws Exception {
		...
		rpcConnection = notificationService.addTransaction(transaction, request);//将某个客户端的请求，根据id，存放到一个集合中

		String sessionId = rpcConnection.getSessionId();
		if (sessionId == null && !ProtocolElements.JOINROOM_METHOD.equals(request.getMethod())) {//防止并发处理时，客户端连接断开或者中断请求出现问题
			log.warn(
	...
	public void joinRoom(RpcConnection rpcConnection, Request<JsonObject> request) {
		...
		HttpSession httpSession = (HttpSession) rpcConnection.getSession().getAttributes().get("httpSession");//获取servlet自己的session

		JsonObject sessions = (JsonObject) httpSession.getAttribute("openviduSessions");
		if (sessions == null) {//分配sessionId,这块代码太烂了
			...
			if (sessions.has(sessionId)) {
				...
				} else if (sessionManager.getSessionNotActive(sessionId) != null) {//已经关闭了session
		...
		boolean generateRecorderParticipant = false;//是不是设置那台客户端为录制机？
		...
		if (sessionManager.isTokenValidInSession(token, sessionId, participantPrivatetId)) {//验证token是否有效

			String clientMetadata = getStringParam(request, ProtocolElements.JOINROOM_METADATA_PARAM);

			if (sessionManager.formatChecker.isServerMetadataFormatCorrect(clientMetadata)) {

				Token tokenObj = sessionManager.consumeToken(sessionId, participantPrivatetId, token);//处理本次请求，从请求列表中去掉
